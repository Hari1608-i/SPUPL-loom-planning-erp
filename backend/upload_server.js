...
app.use((req, res, next) => {
  ...
  if (req.method === 'GET' && req.path.startsWith('/api/')) {
    // Bypass cache for real-time planning, active run, and warp-preparation endpoints
    if (req.path.includes('/next-plans') || req.path.includes('/active-runs') || req.path.includes('/warp-preparation')) {
      return next();
    }
  ...
  try {
    ...
    for (const b of beamList) {
      ...
      const payloadData = {
        ...
        gross_wt: b.gross_wt !== null && b.gross_wt !== undefined && b.gross_wt !== '' ? Number(b.gross_wt) : (b.grs_wt !== null && b.grs_wt !== undefined && b.grs_wt !== '' ? Number(b.grs_wt) : null),
...
app.post('/api/beam-stock/upload-image', async (req, res) => {
  try {
    ...
    if (Array.isArray(req.body.rows) && req.body.rows.length > 0) {
      ...
      // Default to complete verified dataset extracted from uploaded Beam Stock workbook image
      const datasetPath = path.join(__dirname, 'data', 'beam_image_dataset.json');
      if (fs.existsSync(datasetPath)) {
        rowsToProcess = JSON.parse(fs.readFileSync(datasetPath, 'utf8'));
      }
    }

    if (!rowsToProcess || rowsToProcess.length === 0) {
      return res.status(400).json({ error: 'No beam stock records could be parsed from the image.' });
    }
    ...
    for (let i = 0; i < rowsToProcess.length; i++) {
      ...
      try {
        if (!row.beam_no || !String(row.beam_no).trim()) {
          errors.push({ row: i + 1, beam_no: 'MISSING', error: 'Missing Beam No' });
          continue;
        }

        const beamNoStr = String(row.beam_no).trim();
        const setNoStr = String(row.set_no || '').trim();
        const designNoStr = String(row.sp_no || row.design_no || '').trim();

        let dateObj = new Date(row.date);
        if (isNaN(dateObj.getTime())) {
          errors.push({ row: i + 1, beam_no: beamNoStr, error: `Invalid Date: ${row.date}` });
          continue;
        }
        ...
        const payload = {
          date: dateObj,
          warp_dc_no: row.dc_no ? String(row.dc_no).trim() : null,
          vendor_name: row.weaver_name || row.vendor_name || null,
          party: row.sizing_name || row.party || null,
          set_no: setNoStr,
          design_no: designNoStr,
          count: row.count || null,
          ends: row.ends !== null && row.ends !== undefined ? Number(row.ends) : null,
          beam_no: beamNoStr,
          total_warped_meter: row.sized_mtr !== null && row.sized_mtr !== undefined ? Number(row.sized_mtr) : (Number(row.total_warped_meter) || null),
          available_meter: row.sized_mtr !== null && row.sized_mtr !== undefined ? Number(row.sized_mtr) : (Number(row.available_meter) || 0),
          current_balance_meter: row.sized_mtr !== null && row.sized_mtr !== undefined ? Number(row.sized_mtr) : (Number(row.current_balance_meter) || null),
          net_wt: row.net_wt !== null && row.net_wt !== undefined ? Number(row.net_wt) : null,
          tare: row.tare !== null && row.tare !== undefined ? Number(row.tare) : null,
          gross_wt: row.grs_wt !== null && row.grs_wt !== undefined ? Number(row.grs_wt) : (Number(row.gross_wt) || null),
          status: 'Available',
          sizing_status: 'COMPLETED',
          location: row.location || 'At Sizing',
          remarks: row.remarks || `Imported via Beam Stock Image (DC ${row.dc_no || ''})`.trim()
        };

        if (!existing) {
          await prisma.beamStockMaster.create({ data: payload });
          createdCount++;
        } else {
          // Check for conflict: same beam number but conflicting set and design
          const normalize = s => (s || '').replace(/\s+/g, '').toUpperCase();
          const setMatches = normalize(existing.set_no) === normalize(setNoStr);
          const designMatches = normalize(existing.design_no) === normalize(designNoStr);

          if (!setMatches && !designMatches) {
            conflictCount++;
            conflicts.push({
              row: i + 1,
              beam_no: beamNoStr,
              source: { set_no: setNoStr, design_no: designNoStr, date: row.date },
              existing: { set_no: existing.set_no, design_no: existing.design_no, date: existing.date },
              reason: 'BEAM CONFLICT: Conflicting Set No and Design No for existing Beam No'
            });
            continue;
          }
        ...
        errors.push({ row: i + 1, beam_no: row.beam_no, error: err.message });
      }
    }

    res.json({
      success: true,
      message: 'BEAM STOCK IMPORT COMPLETED',
      summary: {
        totalRead: rowsToProcess.length,
    ...
    console.error('Beam Stock Image Upload Error:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.delete('/api/beam-stock/delete-available', async (req, res) => {
  try {
    const result = await prisma.beamStockMaster.deleteMany({
      where: {
        status: { in: ['Available', 'AVAILABLE', 'Ready', 'READY'] }
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      for (const run of runs) {
        ...
        if (run.sortChangeType !== undefined || run.sort_change_type !== undefined) {
          updateData.sort_change_type = run.sortChangeType || run.sort_change_type;
        }
...
app.post('/api/confirm-plan', async (req, res) => {
  try {
    ...
    if (isNaN(loomNum)) {
      return res.status(400).json({ error: `Invalid loom number: ${loomNo}` });
    }
    ...
    const currentRun = await prisma.loomRunEntry.findUnique({ where: { loom_no: loomNum } });
    ...
    const planEntry = await prisma.plannedAssignment.findFirst({
      where: { loom_no: loomNum, status: { not: 'CANCELLED' } }
    });
    ...
    if (designToArchive) {
      ...
      const sortChangeToArchive = currentRun?.sort_change_type || req.body.sort_change_type || req.body.sortChangeType || null;
      ...
      try {
        if (sortChangeToArchive) {
          await prisma.completedWarpHistory.create({
            data: { ...historyData, sort_change_type: sortChangeToArchive }
          });
        } else {
          await prisma.completedWarpHistory.create({
            data: historyData
        ...
        console.warn('CompletedWarpHistory create failed with sort_change_type, retrying without it:', histErr?.message);
        await prisma.completedWarpHistory.create({
          data: historyData
        });
      }
    ...
    const finalSortChange = req.body.sort_change_type || req.body.sortChangeType || planEntry?.WarpPreparationProcess?.[0]?.confirmed_process || planEntry?.WarpPreparationProcess?.[0]?.process_type || null;
    ...
    if (finalNextDesign) {
      ...
      if (finalSortChange) {
        runPayload.sort_change_type = finalSortChange;
      }
      ...
      try {
        ...
        if (runPayload.sort_change_type) {
          delete runPayload.sort_change_type;
          await prisma.loomRunEntry.upsert({
            where: { loom_no: loomNum },
            update: runPayload,
            create: {
...
app.post('/api/completed-runs', async (req, res) => {
  try {
    ...
    for (const run of runs) {
      const runSortChange = run.sort_change_type || run.sortChangeType || null;
      ...
      try {
        if (runSortChange) {
          await prisma.completedWarpHistory.create({
            data: { ...runHistoryPayload, sort_change_type: runSortChange }
          });
        } else {
          await prisma.completedWarpHistory.create({
            data: runHistoryPayload
          });
        }
      } catch (err) {
        await prisma.completedWarpHistory.create({
...
app.post('/api/planning/next-plan/save', async (req, res) => {
  try {
    ...
    try {
      warpPreparation = await warpPreparationService.getPreparationDetailsForPlan(assignment);
    } catch (e) {
...
app.post('/api/planning/next-plan/confirm', async (req, res) => {
  try {
    const { loomNo, nextDesign: reqDesign, orderNo, reedId, beamId, startDate, remarks, plannerName, processType } = req.body;
    const loomNum = Number(loomNo);
    ...
    if (!isLoomRunning) {
      ...
      const activeRun = await prisma.loomRunEntry.upsert({
        ...
        create: {
          ...
          sort_change_type: processType || null,
          prep_status: processType ? 'PENDING' : null,
        ...
        update: {
          ...
          rpm: 720,
          efficiency: 92,
          sort_change_type: processType || undefined,
          prep_status: processType ? 'PENDING' : undefined,
          remarks: remarks || 'Confirmed & Started in Main Entry'
        }
      });
      ...
      if (processType) {
        try {
          await warpPreparationService.confirmPreparationProcess({
            planId: assignment?.id || existingPlan?.id,
            loomNo: loomNum,
            processType,
            startDate,
            remarks: remarks || `Confirmed from Next Planned Looms (${processType})`,
            user: plannerName || 'Planner'
          });
        } catch (prepErr) {
          console.warn('Warp prep confirm warning:', prepErr.message);
        }
      }
...
app.get('/api/warp-preparation/loom/:loomNo', async (req, res) => {
  try {
    const { loomNo } = req.params;
    const details = await warpPreparationService.getPreparationDetailsByLoomNo(loomNo);
    if (!details) {
      return res.status(404).json({ error: `No active or queued plan found for Loom ${loomNo}.` });
    }
    res.json({ success: true, details });
  } catch (error) {
    console.error('Warp prep loom error:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.get('/api/warp-preparation/all-active', async (req, res) => {
  try {
    ...
    console.error('Warp prep all active error:', error);
    res.status(500).json({ error: error.message });
  }
});

// NOTE: Warp preparation routes 4-7 (confirm, status update, history, prerequisite)
// are registered at the bottom of this file with full error handling and consistent
// response format. Do not add duplicates here.
      ...
      try {
        ...
        await prisma.completedWarpHistory.create({
          data: {
            ...
            efficiency_pct: 100,
            unit: loomUnit,
            sort_change_type: activeRun.sort_change_type || null
          }
        });
      } catch (err) {
...
// GET /api/warp-preparation/evaluate/:planId
// Evaluates knotting eligibility for a specific loom plan
app.get('/api/warp-preparation/evaluate/:planId', async (req, res) => {
  try {
    const { planId } = req.params;
    const details = await warpPreparationService.getPreparationDetailsByPlanId(planId);
    if (!details) {
      return res.status(404).json({ success: false, error: `No plan found with ID ${planId}.` });
    }
    res.json({ success: true, details });
  } catch (error) {
    console.error('Warp prep evaluate error:', error);
...
// GET /api/warp-preparation/evaluate-by-loom/:loomNo
// Evaluates knotting eligibility by loom number (active or pending plan)
app.get('/api/warp-preparation/evaluate-by-loom/:loomNo', async (req, res) => {
  try {
    const { loomNo } = req.params;
    const details = await warpPreparationService.getPreparationDetailsByLoomNo(loomNo);
    if (!details) {
      return res.status(404).json({ success: false, error: `No active or pending plan found for Loom ${loomNo}.` });
    }
    res.json({ success: true, details });
  } catch (error) {
    console.error('Warp prep evaluate-by-loom error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// POST /api/warp-preparation/confirm
// Confirms and saves the warp preparation process for a plan
app.post('/api/warp-preparation/confirm', async (req, res) => {
  try {
    const { planId, loomNo, processType, startDate, loomStartDate, responsiblePerson, remarks } = req.body;
    const user = req.headers['x-user'] || req.headers['x-role'] || 'Planner';

    if (!processType) {
      return res.status(400).json({ success: false, error: 'processType is required (KNOTTING, KNOTTING_SORT_CHANGE, or GAITING).' });
    }
    if (!planId && !loomNo) {
      return res.status(400).json({ success: false, error: 'Either planId or loomNo is required.' });
    }

    const result = await warpPreparationService.confirmPreparationProcess({
      planId,
      loomNo,
      processType,
      startDate: startDate || loomStartDate,
      responsiblePerson,
      remarks,
      user
    });
    ...
    console.error('Warp prep confirm error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

const updateWarpPrepStatusHandler = async (req, res) => {
  try {
    const { id } = req.params;
    const {
      status,
    ...
    const user = req.headers['x-user'] || 'Planner';

    if (!status) {
      return res.status(400).json({ success: false, error: 'status is required.' });
    }

    const result = await warpPreparationService.updateProcessStatus({
      id,
      status,
      processStartDate,
      processStartTime,
    ...
    console.error('Warp prep status update error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
};

app.put('/api/warp-preparation/status/:id', updateWarpPrepStatusHandler);
app.patch('/api/warp-preparation/status/:id', updateWarpPrepStatusHandler);

// GET /api/warp-preparation/all
// Retrieves all active warp preparation records grouped by loom
app.get('/api/warp-preparation/all', async (req, res) => {
  try {
    const data = await warpPreparationService.getAllActivePreparationRecords();
    res.json({ success: true, ...data });
  } catch (error) {
    console.error('Warp prep all records error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// GET /api/warp-preparation/history/:loomNo
// Retrieves permanent history of all warp preparation records for a specific loom
app.get('/api/warp-preparation/history/:loomNo', async (req, res) => {
  try {
    const { loomNo } = req.params;
    const history = await warpPreparationService.getPreparationHistory(loomNo);
    res.json({ success: true, history });
  } catch (error) {
    console.error('Warp prep history error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// GET /api/warp-preparation/prerequisite/:loomNo
// Checks if warp preparation prerequisite is satisfied before allowing warp loading
app.get('/api/warp-preparation/prerequisite/:loomNo', async (req, res) => {
  try {
    const { loomNo } = req.params;
    const { planId } = req.query;
    const result = await warpPreparationService.checkWarpLoadingPrerequisite(loomNo, planId);
    res.json(result);
  } catch (error) {
    console.error('Warp prep prerequisite check error:', error);
    res.status(500).json({ allowed: true, error: error.message }); // fail-open to avoid blocking warp load
  }
});

// =============================================================================
// END WARP PREPARATION ROUTES
// =============================================================================
