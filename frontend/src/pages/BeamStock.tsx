...
export default function BeamStock() {
  ...
  // Image Upload / Auto-Update State
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        const mapped: BeamRowState[] = stockOnly.map((b: any, idx: number) => {
          ...
          return {
            ...
            net_wt: b.net_wt !== null && b.net_wt !== undefined ? Number(b.net_wt) : null,
            tare: b.tare !== null && b.tare !== undefined ? Number(b.tare) : null,
            gross_wt: b.gross_wt !== null && b.gross_wt !== undefined ? Number(b.gross_wt) : null
          };
        });
        setRows(mapped);
      } else {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/beam-stock/upload-image`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          image: base64Data,
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || 'Failed to process beam stock image');
      }
      ...
      setImportSummaryModal({
        totalRead: result.summary.totalRead,
      ...
      setSuccessMsg(`BEAM STOCK IMPORT COMPLETED: ${result.summary.totalRead} rows read (${result.summary.created} created, ${result.summary.updated} updated)`);
      setTimeout(() => setSuccessMsg(null), 6000);
    } catch (err: any) {
      console.error(err);
      setErrorMsg(`Image Import Error: ${err.message}`);
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-24">
        ...
        <div className="flex flex-wrap justify-between items-center gap-3 border-b border-slate-200 pb-3 print:hidden">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="px-3.5 py-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-black text-xs rounded-xl shadow-md transition-all flex items-center disabled:opacity-50"
              title="Upload Beam Stock Image to automatically extract and update records"
            >
              <Upload className="w-3.5 h-3.5 mr-1.5" /> {isUploadingImage ? 'Reading Image...' : 'Import Image'}
            </button>
          </div>
        </div>
      ...
      {/* Beam Stock Image Import Summary Modal */}
      {importSummaryModal && (
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2.5 text-center">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div className="text-[10px] uppercase font-bold text-slate-400">Total Rows Read</div>
                <div className="text-xl font-black text-slate-800 mt-0.5">{importSummaryModal.totalRead}</div>
              </div>
              <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                <div className="text-[10px] uppercase font-bold text-emerald-600">Created</div>
                <div className="text-xl font-black text-emerald-700 mt-0.5">{importSummaryModal.created}</div>
              </div>
              <div className="p-3 bg-blue-50 rounded-xl border border-blue-200">
                <div className="text-[10px] uppercase font-bold text-blue-600">Updated</div>
                <div className="text-xl font-black text-blue-700 mt-0.5">{importSummaryModal.updated}</div>
              </div>
              <div className="p-3 bg-purple-50 rounded-xl border border-purple-200">
                <div className="text-[10px] uppercase font-bold text-purple-600">Already Existing</div>
                <div className="text-xl font-black text-purple-700 mt-0.5">{importSummaryModal.existing}</div>
              </div>
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200">
                <div className="text-[10px] uppercase font-bold text-amber-600">Conflicts</div>
                <div className="text-xl font-black text-amber-700 mt-0.5">{importSummaryModal.conflicts}</div>
              </div>
              <div className="p-3 bg-rose-50 rounded-xl border border-rose-200">
                <div className="text-[10px] uppercase font-bold text-rose-600">Errors</div>
                <div className="text-xl font-black text-rose-700 mt-0.5">{importSummaryModal.errors}</div>
              </div>
            </div>

            {/* Conflicts List if any */}
            {importSummaryModal.conflictList && importSummaryModal.conflictList.length > 0 && (
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs space-y-1 max-h-32 overflow-y-auto">
                <div className="font-bold text-amber-900 flex items-center">
                  <AlertTriangle className="w-4 h-4 mr-1 text-amber-600" /> BEAM CONFLICT DETAILS:
                </div>
                {importSummaryModal.conflictList.map((c, i) => (
                  <div key={i} className="text-[11px] text-amber-800">
                    Row {c.row}: Beam #{c.beam_no} — {c.reason}
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Errors List if any */}
            {importSummaryModal.errorList && importSummaryModal.errorList.length > 0 && (
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs space-y-1 max-h-32 overflow-y-auto">
                <div className="font-bold text-rose-900 flex items-center">
                  <AlertCircle className="w-4 h-4 mr-1 text-rose-600" /> ERROR DETAILS:
                </div>
                {importSummaryModal.errorList.map((e, i) => (
                  <div key={i} className="text-[11px] text-rose-800">
                    Row {e.row} (Beam #{e.beam_no}): {e.error}
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setImportSummaryModal(null)}
                className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-md transition-all"
              >
                Close
              </button>
            </div>
